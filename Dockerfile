...
RUN set -ex \
    ...
    && pip install pytz pyasn1 ndg-httpsclient pyOpenSSL redis pandas --constraint ${CONSTRAINT_URL} \
    && pip install "apache-airflow[crypto,celery,cncf.kubernetes,virtualenv,vertica,hive,hdfs,webhdfs,jdbc,mysql,ssh${AIRFLOW_DEPS:+,}${AIRFLOW_DEPS}]==${AIRFLOW_VERSION}" --constraint ${CONSTRAINT_URL} \
