;(function (root, factory) {
    ...
    ;(function ($) {
        ...
        var Selectpicker = function (element, options, e) {
            ...
            if ($select.is('[with-null="true"]')) {
                var placeholder = this.$element.attr('placeholder') || '--请选择--'
        ...
        Selectpicker.DEFAULTS = {
            ...
            maxOptionsText: function (numAll, numGroup) {
                return [
                    '最多可选择{n}项', // (numAll === 1) ? 'Limit reached ({n} item max)' : 'Limit reached ({n} items max)',
                    '最多可选择{n}分组项' // (numGroup === 1) ? 'Group limit reached ({n} item max)' : 'Group limit reached ({n} items max)'
                ]
            },
            ...
            createDropdown: function () {
                ...
                var actionsbox =
                    this.multiple && this.options.actionsBox
                        ...
                        '</div>'
                        : ''
                var editBtn = this.$selectBtb
                    ? '<div class="b-select-create__option">' +
                ...
                var donebutton =
                    this.multiple && this.options.doneButton
                        ...
                        : ''
                var drop =
                    '<div class="form-control btn-group bootstrap-select' +
                    multiple +
                    inputGroup +
                    '">' +
                    ...
                    '<span class="input-group-inner-addon glyphicon glyphicon-chevron-down"></span>' +
                    '</button>' +
            ...
            render: function (updateLi) {
                ...
                if (!this.multiple || title === '--请选择--') {
                    ...
                    for (var i = 0, len = title.length; i < len; i++) {
                        titleTem +=
                            '<span class="b-tag">' +
                            '<span class="b-select__tags-text">' +
                            title[i] +
                            '</span>' +
                ...
                $('.b-tag-close').on('click', function () {
                    ...
                    var selectText = $(this)
                        ...
                        .text()
                    for (
                        var j = 0, selectLen = $('.b-multiple').length;
                        j < selectLen;
                        j++
                    ) {
                        if ($('.b-multiple')[j]) {
            ...
            initSelectOption: function (options) {
                var defaults = {
                    ...
                    template: function (selectObject) {
                        return (
                            '<option value="' +
                            selectObject[options.idKey] +
                            '">' +
                            selectObject[options.nameKey] +
                            '</option>'
                        )
                    }
                }
                ...
                if (select.is('[with-null="true"]')) {
                    var placeholder = this.$element.attr('placeholder') || '--请选择--';
                    select.append('<option value=' + options.nullValue + '>' + placeholder + '</option>');
                }
                for (var i = 0; i < options.data.length; i++) {
                    select.append(options.template(options.data[i]));
                }
                this.options.dropdownAlignRight = options.dropdownAlignRight;
                if (this.options.dropdownAlignRight) {
                    this.$menu.addClass('dropdown-menu-right');
                }
            ...
            ajaxRequest: function (options) {
                ...
                $.ttyAjax({
                    ...
                    success: function (data) {
                        if (data.code === 0) {
                            options.data = data.resultdata
                            $(_this.$element).selectpicker('initSelectOption', options)
                        }
                    }
                })
            },
