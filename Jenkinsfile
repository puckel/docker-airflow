pipeline {
            ...
            steps {
                sh'''#!/bin/bash
                    docker build . -f Dockerfile \
                        --build-arg AIRFLOW_DEPS="datadog,statsd,github_enterprise" \
    ...
    post {
        failure {
            script {
                pagerduty(resolve: false, serviceKey: "b744c608601448da906e1172d2a0e1ba", incDescription: "Airflow failed to build")
            }
        }
        always {
            script {
